	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int index = 0; index <= in.length(); index+=2){
			for(int charCode = 'a'; charCode <= 'z'; charCode++) {
				...
				// if the item isn't in the list, isn't the original string, and
				// (if wordsOnly is true) is a real word, add to the list
				if(!currentList.contains(sb.toString().replace(" ","").trim()) &&
						(!wordsOnly||dict.isWord(sb.toString().replace(" ","").trim())) &&
						!s.equals(sb.toString().replace(" ","").trim())) {
					currentList.add(sb.toString().replace(" ","").trim());
				}
			}
		}
